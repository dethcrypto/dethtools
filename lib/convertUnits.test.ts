...
describe('convertEthUnits', () => {
  ...
  it.skip('performs tiny unit conversion', () => {
    expect(convertEthUnits('0.000000000000000030043311111001', 'eth', 'gwei')).toEqual('0.00000003')
    expect(convertEthUnits('0.000000000000000030043311111001', 'eth', 'wei')).toEqual('30')
  })

  it.skip('performs lossy conversion ', () => {
    // @notice it fails, as currently we're not expecting decimals in such case
    expect(convertEthUnits('0.00000412300033333300012', 'gwei', 'wei')).toEqual('4123')
    expect(convertEthUnits('12', 'gwei', 'eth')).toEqual('0.000000000000004123')
  })

  it('performs untypical conversion', () => {
    expect(convertEthUnits('0.12', 'wei', 'eth')).toEqual('0.00000000000000000012')
  })
})
